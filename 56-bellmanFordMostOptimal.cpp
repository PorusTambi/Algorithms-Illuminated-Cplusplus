...
vector<int> bellmanFord(int V, vector<vector<int>> &edges, int src)
{
    ...
    vector<int> dist(V, infiniteVal);
    vector<int> parent(V, -1);

    //base case initialization
    //set to distance from src to src =0;
    //since dist[src]=0 this will automatically handle cases of shortest path from s to s with no edges
    //negative/positive cycles to src will be handled by for loop for edges
    dist[src] = 0;

    //used to early exit algo
    //if shortest paths don't change b/w iterations
